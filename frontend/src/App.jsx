...
export default function App() {
  ...
  const handleSearch = useCallback(async (q) => {
    ...
    try {
      const isCluster = CLUSTER_KEYWORDS.some(kw => term.toLowerCase().includes(kw))
        || CLUSTER_KEYWORDS.some(kw => raw.toLowerCase().includes(kw));
      const isClass = CLASS_KEYWORDS.some(kw => term.toLowerCase().includes(kw)) && !isCluster;
    ...
    <div style={{ minHeight: '100vh', position: 'relative', fontFamily: "'Georgia', serif" }}>
      ...
      <div style={{ position: 'relative', zIndex: 1, maxWidth: '1000px', margin: '0 auto', padding: '48px 24px 80px' }}>
        ...
        <div style={{ textAlign: 'center', marginBottom: '48px' }}>
          ...
          <p style={{
            color: '#1a3a5a', fontFamily: 'monospace', fontSize: '12px',
            marginTop: '12px', letterSpacing: '0.12em',
          }}>
            {T.sources}
          </p>
        </div>
        ...
        <div style={{
          ...
          {OBJECT_CATEGORIES.map(cat => (
            <div
              ...
              onMouseEnter={e => {
                e.currentTarget.style.background = '#071220';
                e.currentTarget.style.boxShadow = `0 0 20px ${cat.color}22`;
              }}
              onMouseLeave={e => {
                e.currentTarget.style.background = '#040b14';
            ...
            >
              ...
              <div style={{
                color: cat.color, fontFamily: 'monospace', fontSize: '13px',
                fontWeight: 'bold', letterSpacing: '0.05em', marginBottom: '6px',
              }}>
                {lang === 'pt' ? cat.labelPT : cat.labelEN}
              </div>
              <div style={{
                color: '#6a9ab8', fontFamily: 'monospace', fontSize: '11px',
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: '4px' }}>
                {cat.examples.map(ex => (
                  <span
                    ...
                    style={{
                      color: '#3a6a8a', fontFamily: 'monospace', fontSize: '11px',
        ...
        {error && (
          <div style={{
            ...
            {suggestions.length > 0 && (
              <div>
                <div style={{ color: '#3a2a2a', fontFamily: 'monospace', fontSize: '11px', marginBottom: '8px' }}>
                  {T.suggestions}
                </div>
                <div style={{ display: 'flex', gap: '8px', flexWrap: 'wrap' }}>
                  {suggestions.map(s => (
                    <button
                      ...
                      style={{
                        background: '#0a0505', border: '1px solid #2a1a10',
                        color: '#8a5a4a', fontFamily: 'monospace', fontSize: '11px',
                        cursor: 'pointer', padding: '4px 14px', borderRadius: '2px',
                        transition: 'all 0.15s',
                      }}
        ...
        {result?.mode === 'list' && (
          <>
                ...
                {result.results?.map((obj, i) => (
                  ...
                  >
                    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
                      <div>
                        <span style={{ fontFamily: 'Georgia, serif', fontSize: '17px', color: '#d8eeff' }}>
                          {obj.name}
                        </span>
                        <div style={{ color: '#4a9fd4', fontFamily: 'monospace', fontSize: '11px', marginTop: '4px' }}>
                          {obj.object_type}
                        </div>
                      </div>
            ...
            {selectedObj && (
              <ListItemModal obj={selectedObj} onClose={() => setSelectedObj(null)} />
            )}
